...
app.post('/api/products', requireAdmin, upload.single('image'), (req, res) => {
  ...
  const insertQuery = `INSERT INTO products (nameKey, name, price, image, description, featured, active, category) VALUES (?, ?, ?, ?, ?, ?, 1, ?)`;
  db.run(insertQuery, [null, name, price, imageUrl, description || '', featured === '1' ? 1 : 0, category || 'savories'], function(err) {
    if (err) { res.status(500).json({ error: err.message }); return; }
    res.json({ id: this.lastID, name, price, image: imageUrl, description, featured: featured === '1' ? 1 : 0, active: 1, category: category || 'savories' });
  });
});
